...
onMounted(async () => {
  ...
  if (id) {
    ...
    const depid = route.query?.depid;
    if (depid) {
      state.form.depid = depid;
    }
  }
  rdata.depts = await requestClient.get(`/sys/dept/tree`);
    ...
    <el-card class="box-card" style="height: 100%" body-style="height: 100%">
      <template #header>
        <div class="flex justify-between">
          <el-space>用户信息--{{ form.id ? '修改' : '新增' }}</el-space>
          <el-space>
      ...
      <div style="height: calc(100% - 50px); overflow: auto; padding-top: 5px; padding-left: 0; padding-right: 0">
        <el-form :inline="true" ref="formRef" :model="form" label-width="140px" class="zform">
          ...
          <el-form-item label="账号名称" prop="username" :rules="[{ required: true, message: '账号名称不能为空' }]">
            <el-input v-model="form.username" />
          </el-form-item>
          <el-form-item label="用户密码" prop="password" :rules="[{ required: true, message: '用户密码不能为空' }]" v-if="!form.id">
