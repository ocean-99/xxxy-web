      ...
      <el-card style="height: 100%" body-style="height: 100%" class="box-card flex-1 overflow-hidden">
        <template #header>
          <div class="flex justify-between">
            <el-space>
              <el-input style="width: 180px" v-model="state.form.name" placeholder="输入岗位名称回车查询" clearable @keyup.enter="listQuery(state)" />
              <el-button type="primary" @click="listQuery(state)" icon="Search"> 查询</el-button>
            </el-space>
            <el-space>
              <el-button icon="Plus" type="success" plain @click="editRef.open({ depid: state.form.depid })">新增 </el-button>
        ...
        >
          ...
          <el-table-column label="岗位名称" align="center" prop="name" width="180">
            <template #default="scope">
              <span class="link_span" @click="editRef.open({ id: scope.row.id })">{{ scope.row.name }}</span>
            </template>
          </el-table-column>
          <el-table-column label="所属机构/部门" align="center" prop="depna" width="250" />
