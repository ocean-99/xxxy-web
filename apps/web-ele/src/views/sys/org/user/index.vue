...
import PacodEdit from './pacod.vue';
...
const editOpen = (data: any) => {
  switch (state.editway) {
    ...
    case 'T': {
      router.push({ path: '/sys/org/user/edit', query: data });
      break;
    }
  }
};
      ...
      <Pane>
        <el-card class="box-card" style="height: 100%" body-style="height: 100%">
          <template #header>
            <div class="flex justify-between">
              ...
              <el-space>
                ...
                <el-button icon="Plus" v-access:code="['sysorg:user:page']" type="success" plain @click="editOpen({ depid: state.form.depid })">新增 </el-button>
                <el-button icon="Delete" v-access:code="['sysorg:dept:page']" type="danger" plain @click="listDelete(state)" :disabled="state.multiple">删除 </el-button>
              </el-space>
            </div>
          ...
          >
            ...
            <el-table-column label="用户名称" align="center" prop="name" width="116">
              <template #default="scope">
                <span class="link_span" @click="editOpen({ id: scope.row.id })" v-if="scope.row.id !== 'u1'">{{ scope.row.name }}</span>
                <span v-else>{{ scope.row.name }}</span>
              </template>
            </el-table-column>
            <el-table-column label="用户账号" align="center" prop="usnam" width="116" />
            ...
            <el-table-column label="操作" align="center" fixed="right" width="116" class-name="small-padding fixed-width">
              <template #default="scope">
                ...
                <el-tooltip v-if="scope.row.id !== 'u1'" content="重置密码" placement="top">
                  <el-button link type="primary" icon="Key" @click="pacodRef.open({ id: scope.row.id })" />
                </el-tooltip>
              </template>
            </el-table-column>
          </el-table>
