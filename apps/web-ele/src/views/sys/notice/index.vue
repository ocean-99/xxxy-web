  ...
  <Page auto-content-height>
    <el-card class="box-card" style="height: 100%" body-style="height: 100%">
      ...
      >
        ...
        <el-table-column label="公告类型" prop="type" width="86" align="center">
          <template #default="scope">
            <el-tag :type="scope.row.type === 1 ? 'warning' : 'primary'">{{ scope.row.type === 1 ? '通知' : '公告' }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column label="公告状态" prop="avtag" width="86" align="center">
          <template #default="scope">
            <el-tag :type="scope.row.avtag ? 'success' : 'info'">{{ scope.row.avtag ? '正常' : '关闭' }}</el-tag>
          </template>
        </el-table-column>
    ...
    <ModalEdit ref="editRef" @close="editClose" />
  </Page>
</template>
