...
const receInit = async () => {
  const res = await requestClient.get(`/wf/bus/tmpl/list`);
...
async function nodeClick(node: any) {
  if (node && node.id) {
    tierItems.value = await requestClient.get(`/wf/bus/tmpl/list?catid=${node.id}`);
  }
}
...
const openModal = async (data: any) => {
  state.show = true;
  tierItems.value = await requestClient.get(`/wf/bus/tmpl/list`);
};
          ...
          <el-row>
            <el-col :span="12">
              <div style="height: 390px">
                <CateTree @node-click="nodeClick" url="/wf/bus/cate/tree" />
              </div>
            </el-col>
            <el-col :span="12">
