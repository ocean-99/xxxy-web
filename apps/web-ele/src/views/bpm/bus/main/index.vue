...
import TmplModal from '#/views/bpm/bus/tmpl/select.vue';
...
onMounted(async () => {
  await listQuery(state);
  emitter.on('flowMainEditClose', refresh);
});

onUnmounted(() => emitter.off('flowMainEditClose'));
...
function handleView(row: any) {
  router.push({ path: `${state.url}/view`, query: { id: row.id } });
}

const flowTmplModal = ref();

const openFlowTmplModal = () => {
  flowTmplModal.value.openModal();
};

const closeFlowTmplModal = (item: any) => {
  console.log(item);
  if (item.id) {
    router.push({ path: `${state.url}/edit`, query: { tmpid: item.id, tmpna: item.name } });
  } else {
    ElMessage.warning('请先选择流程模板再发起流程');
  }
};
// endregion
...
<template>
  <Page auto-content-height>
    <div class="flex h-full gap-[8px]">
      <div class="w-[260px]">
        <CateTree @node-click="nodeClick" url="/bpm/bus/tmpl/tree" tip="模板名称" />
      </div>
      <el-card style="height: 100%" body-style="height: 100%" class="box-card flex-1 overflow-hidden">
        ...
        >
          ...
          <el-table-column label="流程标题" prop="name" min-width="200">
            <template #default="scope">
              <span class="link_span" @click="handleView({ id: scope.row.id })">{{ scope.row.name }}</span>
            </template>
          </el-table-column>
          ...
          <el-table-column label="状态" prop="state" width="70" align="center">
            <template #default="scope">
              <el-tag v-show="scope.row.state === '10'" type="warning" size="small">草稿</el-tag>
              <el-tag v-show="scope.row.state === '11'" type="danger" size="small">驳回</el-tag>
              <el-tag v-show="scope.row.state === '20'" type="primary" size="small">待审</el-tag>
          ...
          <el-table-column label="操作" fixed="right" align="center" width="118" class-name="small-padding fixed-width">
            <template #default="scope">
              <el-tooltip content="查看" placement="top">
                <el-button link type="primary" icon="View" @click="handleView({ id: scope.row.id })" />
              </el-tooltip>
    ...
    <TmplModal ref="flowTmplModal" @close="closeFlowTmplModal" />
  </Page>
</template>
