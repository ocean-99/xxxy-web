...
import { putLineNum, putNodeNum } from './customModeler/custom/store';
...
onMounted(async () => {
  ...
  if (props.prdid) {
    const defData = (await requestClient.get(`/wf/def/${props.prdid}`)) as any;
    bpmnModeler.value.importXML(defData.dixml, (err: any) => {
      if (err) {
        console.error(err);
      } else {
        console.log('ok');
    ...
    bpmnModeler.value.importXML(defaultXml, (err: any) => {
      if (err) {
        console.error(err);
      } else {
        console.log('ok');
  ...
  events.forEach((event) => {
    bpmnModeler.value.on(event, (e) => {
      ...
      if (event === 'shape.added') {
        if (e.element.id.slice(0, 1) === 'N' && e.element.id.slice(0, 2) !== 'NS' && e.element.id.slice(0, 2) !== 'NE') {
          // shape.name="审批节点"
          putNodeNum(state.numFlag, e.element.id.slice(1) - 0);
        }
      } else if (event === 'connection.added') {
        // console.log("创建了线段");
        // console.log(e.element);
        if (e.element.id.slice(0, 1) === 'L') {
          putLineNum(state.numFlag, e.element.id.slice(1) - 0);
        } else {
          // 如果更新Line的ID?
