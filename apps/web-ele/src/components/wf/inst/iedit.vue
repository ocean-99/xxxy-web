...
const initInfo = async () => {
  const initVo = (await requestClient.get(`/wf/initInfo?prdid=${props.prdid}&busty=${props.busty}`)) as any;
...
const toggleFlowChart = async () => {
  if (!state.xml) {
    state.xml = (await requestClient.get(`/wf/initXml?prdid=${props.prdid}`)) as any;
