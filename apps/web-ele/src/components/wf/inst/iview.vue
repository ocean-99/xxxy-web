...
// import Pmodal from '#/components/wf/pmodal.vue';
...
const viewInfo = async () => {
  const viewVo = (await requestClient.get(`/wf/viewInfo?proid=${props.proid}`)) as any;
  ...
  if (state.cutag) {
    const passInfo: any = await requestClient.get(`/wf/passInfo?proid=${props.proid}&facno=${form.value.facno}&busty=${props.busty}`);
...
const toggleFlowChart = async () => {
  if (!state.xml) {
    // const result = (await requestClient.get('/wf/zproc?proid=' + props.proid)) as any;
    //
    ...
    const map = (await requestClient.get(`/wf/viewXml?proid=${props.proid}`)) as any;
    state.xml = map.xml;
...
const opChange = async (key: any) => {
  if (key === 'refuse') {
    state.refNodes = await requestClient.get(`/wf/refuseInfo?proid=${props.proid}&facno=${form.value.facno}`);
    ...
    state.ccmen = await requestClient.get(`/wf/ccInfo?proid=${props.proid}`);
  }
};
