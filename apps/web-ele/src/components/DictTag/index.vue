...
const unmatch = computed(() => {
  if (props.options?.length === 0 || props.value === '' || props.value === null || props.value === undefined) return false;
  // 传入值为非数组
...
const unmatchArray = computed(() => {
  // 记录未匹配的项
  const itemUnmatchArray: Array<number | string> = [];
...
const handleArray = (array: Array<number | string>) => {
    ...
    <template v-for="(item, index) in options">
      <template v-if="values.includes(item.value)">
        ...
        <el-tag
          ...
          :index="index"
          :type="item.elTagType === 'primary' || item.elTagType === 'success' || item.elTagType === 'info' || item.elTagType === 'warning' || item.elTagType === 'danger' ? item.elTagType : 'primary'"
