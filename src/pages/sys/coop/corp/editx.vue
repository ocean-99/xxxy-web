<template>
	<div>
				...
				<div style='padding: 20px;'>
					<el-form ref='formRef' class='xform' :inline='true' :model='form' label-width='160px'>
						<div class='zform-div'>
							...
							<el-form-item label='销售机构：' prop='brana' :rules="[{ required: true, message: '机构不能为空'}]" style='width: 50%'>
								<el-input v-model='form.brana' readonly @click='openBranchModal'/>
							</el-form-item>
		...
		<BranchModal url='/sa/org/branch/treea' ref='branchModal' @close='branchChange' />
	</div>
</template>
...
const openBranchModal = () => {
	branchModal.value.openModal();
};

const branchChange = async (node: any) => {
	form.value.braid = node.id;
	form.value.brana = node.name;
};
