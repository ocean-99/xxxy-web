						...
						<el-row style='border-top: 1px solid #d2d2d2;'>
							...
							<el-col :span='12'>
								<el-form-item label='上级部门：'>
									<div class='zinput'>
										<el-input v-model='form.panam' @click='openOrgModal' readonly :suffix-icon='Search'></el-input>
									</div>
								</el-form-item>
							</el-col>
						</el-row>
...
onMounted(async () => {
	...
	if(form.value.parent){
		form.value.panam = form.value.parent.name;
	}
});
...
const closeOrgModal = (data: any) => {
	if (data.orgs && data.orgs.length > 0) {
		...
		form.value.panam = data.orgs[0].name;
