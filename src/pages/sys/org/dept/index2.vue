      ...
      <el-card class='box-card'>
        <template #header>
					<div class='zjustify'>
						...
						<div>
<!--							<el-button type='success' :icon='Plus' @click='tabAdd(state.url,{pid:state.form.pid,pname:state.form.pname})' plain>新增[新页面]</el-button>-->
							<el-button type='success' :icon='Plus' @click='editRef.open({pid:state.form.pid,pname:state.form.pname})' plain>新增 [抽屉]</el-button>
							<el-button type='success' :icon='Plus' @click='tabAdd(state.url,{pid:state.form.pid,pname:state.form.pname},{flush})' plain>
								新增 [页签]
							</el-button>
...
<!--              <span style='cursor:pointer;color: #3e9ece' @click='tabEdit(state.url,scope.row.id)'>{{ scope.row.name }}</span>-->
          ...
          <el-table-column label='更新时间' prop='uptim' width='160'/>
					<el-table-column label='操作' width='76' header-align='center' align="center">
						<template #default="scope">
        ...
        />
			...
			<Dedit ref='editRef' @listQuery='listQuery(state)'/>
    </div>
  </Wrap>
</template>
<script lang='ts'>
export default {name: 'SysOrgDeptIndex2'};
</script>
<script lang='ts' setup>
import {Search,Plus, Delete, ArrowDown, ArrowUp,Operation,Edit} from '@element-plus/icons-vue';
