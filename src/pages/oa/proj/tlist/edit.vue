<template>
  <el-dialog v-model='state.show' title='任务清单编辑' draggable width='500px'>
    <el-form ref='formRef' :model='form' label-width='120px'>
      <el-form-item label='清单名称：' prop='name' :rules="[{ required: true, message: '策略名称不能为空'}]">
...
const openModal = async (data: any) => {
  if (data && data.id) {
    ...
    state.form = {avtag: true,proid:data.proid};
