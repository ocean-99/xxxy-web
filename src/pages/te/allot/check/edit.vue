<template>
	<el-card class='box-card' :body-style="{padding:'2px 8px'}" shadow='never'>
		<template #header>
			<el-row>
				<el-col :span='10'>
					<div style='line-height: 32px'>校验分配</div>
				</el-col>
		...
		<div style='margin-top: 8px;margin-bottom: 8px'>
			<el-form ref="formRef" class='zform' :model='form' label-width='140px'>
				<el-tabs type='card' v-model='activeName'>
					...
					<el-tab-pane label='校验分配' name='tab2'>
						<div style='margin-left: 15px;margin-right: 15px'>
							<el-table ref='listTableRef' :data='checks' style='width: 100%;height: 500px'>
								<el-table-column type='selection' width='55' />
								<el-table-column prop='name' label='校验名称' width='180' />
...
<script lang='ts'>
export default { name: 'teAllotCheck' };
</script>
...
const state = reactive({
	url: '/te/allot/check', cates: [] as any,
	params: { path: '', query: '' }, checks: [] as any,
	form: { avtag: true, items: [] as any } as any,
});

const { form, cates, checks } = toRefs(state);
...
const save = async () => {
	...
	for (let i = 0; i < listTableRef.value.getSelectionRows().length; i++) {
		form.value.items.push({ cheid: listTableRef.value.getSelectionRows()[i].id });
	}
...
const cateChange = async () => {
	checks.value = await request({
		url: '/te/allot/check/checks/',
		method: 'get',
		params: { id: form.value.catid },
	});

	await nextTick(() => {
		checks.value.forEach((item: any) => {
			for (const fitem of form.value.items) {
				if (fitem.cheid == item.id) {
