			...
			<div style='padding: 20px;border-top: 1px solid #ccc'>
				<el-form ref='formRef' :model='form' label-width='160px'>
					<el-row style='height: 48px'>
						<el-col :span='12'>
							<el-form-item label='型号名称：' prop='name' :rules="[{ required: true, message: '名称不能为空'}]">
						...
						<el-col :span='6'>
							<el-form-item label='所属系列：' prop='serid'>
								<el-select v-model='form.serid' placeholder='请选择产品系列'>
									<el-option
...
const state = reactive({
	url: '/te/prod/model', show: false, cates: [] as any, series: [] as any,
	form: { avtag: true } as any,
});

const { form, cates, series } = toRefs(state);
...
const cateChange = async () => {
	series.value = await request({
		url: '/te/prod/serie/list',
		method: 'get',
