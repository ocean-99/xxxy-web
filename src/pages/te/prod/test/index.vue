<template>
	<div>
		<ProdModal ref='prodModal' @close='prodChoose' />
		<el-card class='box-card'>
			...
			<el-table height='400' :cell-style="{padding:'2px'}" :row-style="{height: '36px'}" v-loading='state.loading' :data='state.list'
				...
				<el-table-column label='测试名称' width='180'>
...
import { listQuery, listDelete, listSelect, tabEdit } from '/@/comps/page/index';
import ProdModal from './ProdModal.vue';
import router from '/@/router';
import { uuid } from '/@/utils/xutil';

const state = reactive({
	url: '/te/prod/test', loading: true, ids: [], moreParams: false,
...
const prodModal = ref();
const add = () => {
	prodModal.value.openModal();
};
const prodChoose = (node: any) => {
	router.push({
		path: state.url + '/edit',
		query: { uuid: uuid(), modid: node.id },
	});
};
</script>
