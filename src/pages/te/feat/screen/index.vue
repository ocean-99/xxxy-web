	...
	<div>
		<el-card class='box-card'>
			<template #header>
				<el-row>
					<el-col :span='14'>
						<el-select v-model='state.form.catid' clearable placeholder='产品分类' style='margin-right:10px;width: 120px'>
							<el-option
			...
			<el-table height='400' :cell-style="{padding:'2px'}" :row-style="{height: '36px'}" v-loading='state.loading' :data='state.list'
				...
				<el-table-column label='屏幕名称' width='180'>
...
<script lang='ts'>
export default { name: 'teFeatScreen' };
</script>
...
const state = reactive({
	url: '/te/feat/screen', loading: true, ids: [], cates: [] as any,
