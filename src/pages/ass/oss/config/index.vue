  ...
  <div>
    <el-card class='box-card'>
      ...
      <el-table height='400' :cell-style="{padding:'2px'}" :row-style="{height: '36px'}"
        ...
        <el-table-column label='配置key' prop='id' width='160'>
          <template #default='scope'>
						<span style='cursor:pointer;color: #3e9ece' @click='editRef.openModal({id:scope.row.id})'>
							{{ scope.row.config_key }}
						</span>
          </template>
        </el-table-column>
        <el-table-column label='访问站点' prop='endpoint' width='180'/>
        <el-table-column label='桶名称' prop='bucket_name' width='160'/>
        <el-table-column label='前缀' prop='prefix' width='160'/>
        <el-table-column label='状态' prop='statue' width='160'/>
...
const state = reactive({
  url: '/ass/oss/config', loading: true, ids: [], cates: [] as any,
