<template>
  <el-card class='box-card' :body-style="{padding:'2px 8px'}" shadow='never'>
    <template #header>
      <el-row>
        <el-col :span='10'>
          <div style='line-height: 32px'>模型图谱维护</div>
        </el-col>
      ...
      <el-form ref='formRef' :inline='true' class='yform' :model='form' label-width='140px'>
        <el-tabs type='card' v-model='activeName'>
          <el-tab-pane label='基本信息' name='tab1'>
            <div class='yform-div'>
              <el-form-item label='模型图谱名称：' prop='name' style='width:100%' :rules="[{ required: true, message: '名称不能为空'}]">
          ...
          <el-tab-pane label='图谱配置' name='tab2'>
            <div style='width: 100%;height: 720px'>
              <!--              <Graph @getXml='getXml' ref="graphRef"/>-->
    ...
    <TableModal ref='tableModal' @close='closeTableModal'/>
    <Amap ref='amapRef' @close='closeAmap'/>
  </el-card>
</template>
<script lang='ts'>
export default {name: 'BiJmGraphEdit}'};
</script>
...
const state = reactive({
  url: '/bi/model/map', cates: [] as any, grades: [] as any,
  params: {path: '', query: ''},
...
const openTableModal = () => {
  tableModal.value.openModal({
    opener: 'opman',
    orgType: 8,
  });
};

const closeTableModal = (data:any) => {
