  ...
  <div>
    <el-card class='box-card'>
      ...
      <el-table height='400' :cell-style="{padding:'2px'}" :row-style="{height: '36px'}"
        ...
        <el-table-column label='模型仓库名称' width='160'>
          <template #default='scope'>
						<span style='cursor:pointer;color: #3e9ece' @click='toDesignEdit(scope.row.id)'>
							{{ scope.row.name }}
						</span>
          </template>
        </el-table-column>
...
const state = reactive({
  url: '/bi/model/base', loading: true, ids: [],
...
const toDesignEdit=async (basid:string)=>{
  await router.push({
