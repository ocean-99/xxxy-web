			...
			<el-table height='400' :cell-style="{padding:'2px'}" :row-style="{height: '36px'}" v-loading='state.loading' :data='state.list'
				...
				<el-table-column label='编号' width='300'>
					<template #default='scope'>
						<span style='cursor:pointer;color: #3e9ece' @click='drawerRef.open(scope.row.id)'>{{ scope.row.id }}</span>
					</template>
				</el-table-column>
				<el-table-column label='任务名称' prop='name' width='160' />
				<el-table-column label='开始时间' prop='sttim' width='160' />
				<el-table-column label='结束时间' prop='entim' width='160' />
				<el-table-column label='是否成功' prop='state' width='120' />
...
const state = reactive({
	url: '/bi/trans/log', loading: true, ids: [],
