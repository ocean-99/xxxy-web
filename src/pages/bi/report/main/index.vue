<template>
	<Wrap>
		<div class='p-left'>
			<CateTree url='/bi/report/cate/tree' @node-click='nodeClick' />
		</div>
		<div class='p-right'>
			<el-card class='box-card'>
				<template #header>
					<el-row>
						...
						<el-col :span='10' style='text-align: right'>
							<el-button type='success' :icon='Plus' @click='tabAdd(state.url,{catid:state.form.catid,catna:state.form.catna})' plain>新增</el-button>
					...
					<div v-show='state.moreParams' class='more-params'>
						<el-form :inline='true' label-width='100px'>
							<el-form-item label='xx'>
								<el-input v-model='state.form.xx' placeholder='xx' style='width:120px' />
							</el-form-item>
							<el-form-item label='yy'>
								<el-input v-model='state.form.yy' placeholder='yy' style='width:120px' />
							</el-form-item>
							<el-form-item />
						</el-form>
					</div>
				...
				<el-table height='400' :cell-style="{padding:'2px'}" :row-style="{height: '36px'}" v-loading='state.loading' :data='state.list'
					...
					<el-table-column label='报表名称' width='180'>
...
const state = reactive({
	url: '/bi/report/main', loading: true, ids: [], moreParams: false,
